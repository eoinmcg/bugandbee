...
export default class Slope extends Sprite {
  ...
  update() {
    ...
    if (this.pos.x < this.g.size.min.x - (this.size.x / 2)) {
      ...
      this.pos.x = this.g.size.max.x + (this.size.x / 1);
  ...
  render() {
    ...
    if (this.type === 'bottom') {
      drawRect(pos.add(vec2(0, -5)), size.add(vec2(.5, 10)), this.col, this.angle);
    }

    const xOff = size.x + 1 + Math.abs(this.g.angle * 10)
