...
async function engineInit(
  ...
  // no images to load
  if (!imageSources.length) promises.push(loadTexture(0));
...
function engineObjectsUpdate() {
  ...
  function updateChildObject(o) {
    ...
    o.update();
    for (const child of o.children) updateChildObject(child);
  }
...
function engineObjectsDestroy(immediate = true) {
  for (const o of engineObjects) o.parent || o.destroy(immediate);
  engineObjects = engineObjects.filter((o) => !o.destroyed);
}
...
function drawEngineLogo(t) {
  ...
  const gradient = (X1, Y1, X2, Y2, C, S = 1) => {
    if (C >= 0) {
      if (blackAndWhite) x.fillStyle = "#fff";
      else {
  ...
  const poly = (points, C, Y, H) => {
    x.beginPath();
    for (const p of points) x.lineTo(p.x, p.y);
    x.closePath();
    gradient(0, Y, 0, Y + H, C);
  };
  const color = (c, l) =>
    l ? `hsl(${[0.95, 0.56, 0.13][c % 3] * 360} 99%${[0, 50, 75][l]}%` : "#000";
  ...
  if (showName) {
    ...
    for (let j = 2; j--; )
      for (let i = 0, X = 40 - w2 / 2; i < s.length; ++i) {
        ...
        x[j ? "strokeText" : "fillText"](s[i], X2, Y + 0.5, 17 * p);
        X += w;
      }
  ...
  // wheels
  for (let i = 3; i--; )
    for (let j = 2; j--; ) circle(17 + 15 * i, 47, j ? 7 : 1, 0, 2 * PI, 2);
...
function debugUpdate() {
  ...
  if (debugVideoCaptureIsActive()) {
    ...
    if (!debugOverlay || keyWasPressed("Digit6")) debugVideoCaptureStop();
  } else if (debugOverlay && keyWasPressed("Digit6")) debugVideoCaptureStart();
}
...
function debugRender() {
  ...
  if (debugGamepads && gamepadsEnable) {
    ...
    for (let i = 0; i < maxGamepads; i++) {
      if (!gamepadConnected(i)) continue;

      const stickScale = 1;
      const buttonScale = 0.2;
      const cornerPos = cameraPos.add(
        vec2(
          -stickScale * 2,
          ((gamepadConnectedCount - 1) / 2 - i) * stickScale * 3,
        ),
      );
      ...
      for (let j = 0; j < stickCount; j++) {
        ...
        debugText(j, drawPos, 0.3);
        debugPoint(stickPos, "#f00");
      }

      const buttonCount = inputData[i + 1].length;
      for (let j = 0; j < buttonCount; j++) {
        const drawPos = cornerPos.add(
          vec2(j * buttonScale * 2, -stickScale - buttonScale * 2),
        );
...

  {
    ...
    if (debugOverlay) {
      ...
      mousePressed &&
        debugContext.fillText("Mouse: " + mousePressed, x, (y += h));
      ...
      for (let i = 1; i < inputData.length; i++) {
        ...
        if (inputData[i])
          for (const j in inputData[i]) {
            if (keyIsDown(j, i)) buttonsPressed += j + " ";
          }
        buttonsPressed &&
          debugContext.fillText(
            `Gamepad ${i - 1}: ` + buttonsPressed,
            x,
            (y += h),
          );
      }
...
function debugVideoCaptureStop() {
  ...
  // stop recording
  LOG(
    `Video capture ended. ${debugVideoCapture.captureTimer.get().toFixed(2)} seconds recorded.`,
  );
...
function debugVideoCaptureUpdate() {
  ...
  debugVideoCaptureIcon.textContent =
    "● REC " + formatTime(debugVideoCapture.captureTimer);
}
...
function oscillate(
  ...
  if (type === 1)
    ...
    // sawtooth
    value = 2 * phase - 1; // sine
  else value = -cos(phase * 2 * PI);
 ...
 * @memberof Math */
function isString(s) {
  return s != null && typeof s?.toString() === "string";
}
...
function lineTest(posStart, posEnd, testFunction, normal) {
  ...
  while (t < totalLength) {
    if (testFunction(pos)) {
      ...
      if (hitPosFloor.x < pos.x) hitPos.x = pos.x;
      else if (hitPosFloor.x > pos.x) hitPos.x = pos.x + 1 - e;
      if (hitPosFloor.y < pos.y) hitPos.y = pos.y;
      else if (hitPosFloor.y > pos.y) hitPos.y = pos.y + 1 - e;
...
class Vector2 {
  ...
  snap(grid) {
    ...
    return new Vector2(
      floor(this.x * grid) / grid,
      floor(this.y * grid) / grid,
    );
  }
...
function saveCanvas(canvas, filename = "screenshot", type = "image/png") {
  ...
  } else saveDataURL(canvas.toDataURL(type), filename);
}
 ...
 *  @param {number} aspect
 *  @memberof Settings */
function setCanvasMinAspect(aspect) {
  canvasMinAspect = aspect;
}

/** Set maximum aspect ratio of the canvas (width/height), unused if 0
 *  @param {number} aspect
 *  @memberof Settings */
function setCanvasMaxAspect(aspect) {
  canvasMaxAspect = aspect;
}
 ...
 *  @memberof Settings */
function setTileDefaultPadding(padding) {
  tileDefaultPadding = padding;
}

/** Default amount of pixels smaller to draw tiles to prevent neighbor bleeding
 *  @param {number} bleed
 *  @memberof Settings */
function setTileDefaultBleed(bleed) {
  tileDefaultBleed = bleed;
}
 ...
 *  @param {number} time
 *  @memberof Settings */
function setTouchGamepadDisplayTime(time) {
  touchGamepadDisplayTime = time;
}
 ...
 *  @param {boolean} show
 *  @memberof Debug */
function setDebugWatermark(show) {
  debugWatermark = show;
}
...
class EngineObject {
      ...
      for (const o of engineObjectsCollide) {
        ...
        if (smallStepUp || isBlockedY || !isBlockedX) // resolve y collision
        {
          ...
          if ((o.groundObject && wasFalling) || !o.mass) {
            ...
            if (wasFalling) this.groundObject = o;
  ...
  renderDebugInfo() {
    ...
    const hasPhysics =
      this.collideTiles || this.collideSolidObjects || this.isSolid;
    ...
    if (this.parent)
      debugRect(this.pos, size.scale(0.8), rgb(1, 1, 1, 0.5), 0, this.angle);
    this.parent && debugLine(this.pos, this.parent.pos, rgb(1, 1, 1, 0.5), 0.5);
  }
}
/**
...
function tile(
  ...
  texture = 0,
  padding = tileDefaultPadding,
  bleed = tileDefaultBleed,
) {
  ASSERT(
    isVector2(index) || typeof index === "number",
    "index must be a vec2 or number",
  );
  ASSERT(
    isVector2(size) || typeof size === "number",
    "size must be a vec2 or number",
  );
  ASSERT(
    isNumber(texture) || texture instanceof TextureInfo,
  ...
  const textureInfo =
    typeof texture === "number" ? textureInfos[texture] : texture;
...
function isOnScreen(pos, size = 0) {
  ...
  if (size instanceof Vector2) size = size.length(); // use length of vector as diameter
  size *= cameraScale;

  // check against screen bounds
  const w = mainCanvasSize.x,
    h = mainCanvasSize.y;
  return x + size > -w && x - size < w && y + size > -h && y - size < h;
}
...
class FontImage {
  ...
  drawText(text, pos, size = 1, center, color, useWebGL, context) {
    ...
    if (typeof size === "number") {
      ...
      size *= cameraScale;
      size = new Vector2(size, size);
    } else size = size.scale(cameraScale);
    this.drawTextScreen(
      text,
      worldToScreen(pos),
      size,
  ...
  drawTextScreen(
    ...
    (text + "").split("\n").forEach((line, j) => {
      ...
      for (let i = line.length; i--; ) {
        ...
        const index = charCode < 32 || charCode > 127 ? 95 : charCode - 32; // handle out of range characters
...
function vibrate(pattern = 100) {
  ASSERT(
    isNumber(pattern) || isArray(pattern),
    "pattern must be a number or array",
  );
...
function inputInit() {
  ...
  function onKeyDown(e) {
    ...
    const preventDefaultKeys = [
      ...
      "ArrowDown",
      "ArrowLeft",
      "ArrowRight", // scrolling
      "Space", // page down scroll
      "Tab", // focus navigation
      "Backspace", // browser back
    ];
    if (preventDefaultKeys.includes(e.code) || printable) e.preventDefault();
...
function inputUpdate() {
  ...
  function gamepadsUpdate() {
    ...
    if (touchGamepadEnable && isTouchDevice) {
      ...
      if (touchGamepadAnalog) sticks[0] = applyDeadZones(leftTouchStick);
      else if (leftTouchStick.lengthSquared() > 0.3) {
    ...
    for (let i = 0; i < gamepadCount; ++i) {
      ...
      for (let j = gamepad.buttons.length; j--; ) {
        ...
        if (button.pressed && (!button.value || button.value > 0.9))
          hadInput = true;
      }
      ...
      if (hadInput) {
        ...
        if (!gamepadHadInput[gamepadPrimary]) gamepadPrimary = i;
...
function inputRender() {
  ...
  function touchGamepadRender() {
    ...
    if (
      !touchGamepadEnable ||
      (!touchGamepadTimer.isSet() && touchGamepadDisplayTime)
    )
      return;

    // fade off when not touching or paused
    const alpha = touchGamepadDisplayTime
      ? percent(
...
class Sound {
  ...
  constructor(
    ...
    taper = soundDefaultTaper,
    onloadCallback,
  ) {
    if (!soundEnable || headlessMode) return;

    ASSERT(
      !asset || isArray(asset) || isString(asset),
      "asset must be a file name or zzfx array",
    );
    ASSERT(
      randomness === undefined || isNumber(randomness),
      "randomness must be a number",
    );
    ASSERT(
      randomness === undefined || (randomness >= 0 && randomness <= 1),
    ...
    if (Array.isArray(asset)) {
      ...
      const defaultRandomness = randomness ?? 0.05;
...
function tileCollisionTest(
  ...
  for (const layer of tileCollisionLayers) {
    if (!solidOnly || layer.isSolid)
      if (layer.collisionTest(pos, size, callbackObject)) return layer;
  }
}
...
function tileCollisionRaycast(
  ...
  for (const layer of tileCollisionLayers) {
    if (!solidOnly || layer.isSolid) {
      const hitPos = layer.collisionRaycast(
        posStart,
        posEnd,
        callbackObject,
        normal,
...
class TileLayer extends CanvasLayer {
  ...
  redrawStart(clear = false) {
    ...
    cameraPos = this.size.multiply(tileSize).scale(0.5);
    cameraScale = 1;

    // set render target to this layer
    this.isUsingWebGL = this.hasWebGL();
    if (this.isUsingWebGL) glSetRenderTarget(this.textureInfo.glTexture, clear);
    else {
      // disable smoothing for pixel art
  ...
  redrawEnd() {
    ...
    if (this.isUsingWebGL) glSetRenderTarget();
  ...
  drawLayerTile(
    ...
    const drawPos = pos.add(size.scale(0.5));
    drawTile(
      drawPos,
      size,
      tileInfo,
  ...
  clearLayerRect(pos, size) {
    ...
    const x = pos.x,
      y = this.canvas.height - pos.y - size.y;
    const useWebGL = this.hasWebGL();
    if (useWebGL) glClearRect(x, y, size.x, size.y);
    else this.context.clearRect(x, y, size.x, size.y);
  }
}
...
class TileCollisionLayer extends TileLayer {
  ...
  collisionTest(pos, size = new Vector2(), callbackObject) {
    ...
    ASSERT(
      !callbackObject ||
        typeof callbackObject === "function" ||
        callbackObject instanceof EngineObject,
      "callbackObject must be a function or EngineObject",
    );

    // make function to check for collision
    const collisionTest = callbackObject
      ? typeof callbackObject === "function"
    ...
    for (let y = minY; y < maxY; ++y)
      for (let x = minX; x < maxX; ++x) {
        ...
        if (
          tileData &&
          collisionTest(tileData, hitPos.set(x + this.pos.x, y + this.pos.y))
        )
          return true;
      }
    return false;
  ...
  collisionRaycast(posStart, posEnd, callbackObject, normal) {
    ...
    const testFunction = (pos) => {
      const tileData = this.getCollisionData(
    ...
    if (debugRaycast && hitPos) {
      ...
      debugLine(posStart, posEnd, "#00f", 0.02);
      debugLine(posStart, hitPos, "#f00", 0.02);
      debugPoint(hitPos, "#0f0");
      normal && debugLine(hitPos, hitPos.add(normal), "#ff0", 0.02);
    }
    return hitPos;
  }
}
...
class ParticleEmitter extends EngineObject {
  ...
  constructor(
    ...
    this.emitSize =
      typeof emitSize === "number" ? vec2(emitSize) : emitSize.copy();
  ...
  update() {
    ...
    } else if (this.particles.length === 0) this.destroy(true);
    ...
    if (debugParticles) {
      // show emitter bounds
      if (this.emitCircle) debugCircle(this.pos, this.emitSize.x / 2, "#0f0");
      else debugRect(this.pos, this.emitSize, "#0f0", 0, this.angle);
    }
  }
  ...
  emitParticle() {
    ...
    // build particle
    const velocity = vec2(
      speed * sin(velocityAngle),
      speed * cos(velocityAngle),
    );
...
class Particle {
  ...
  update() {
    ...
    // check collision against tiles
    this.groundObject = undefined;
    const testCollision = collideCallback
      ? (pos) => {
        ...
        if (!collideCallback || collideCallback?.(this, hitLayer)) {
          ...
          if (isBlockedY || !isBlockedX) {
            const wasFalling =
              (this.velocity.y < 0 && gravity.y < 0) ||
              (this.velocity.y > 0 && gravity.y > 0);
            if (wasFalling) this.groundObject = hitLayer;
  ...
  render() {
    ...
    if (trailScale) {
      ...
      const velocity = localSpace
        ? this.velocity.rotate(-emitter.angle)
        : this.velocity;
...
function glInit(rootElement) {
  ...
  function initWebGL() {
    ...
    ((offset = 0),
      (shader = glPolyShader),
      (stride = gl_POLY_VERTEX_BYTE_STRIDE));
...
function glPreRender(clear = true) {
  ...
  if (glRenderTarget) s.y = -s.y; // invert y when using render target
...
class PostProcessPlugin {
  ...
  constructor(shaderCode, includeMainCanvas = false, feedbackTexture = false) {
    ...
    ASSERT(
      !(includeMainCanvas && feedbackTexture),
    ...
    this.vao = undefined;
    /** @property {boolean} - Enable/disable post processing */
    this.enabled = true;
...
class UISystemPlugin {
  ...
  constructor(context = mainContext) {
    ...
    function uiUpdate() {
      ...
      if (!navigableObjects.length) uiSystem.navigationObject = undefined;
        ...
        if (!isTouchDevice)
          if (uiSystem.navigationMode && !uiSystem.navigationObject) {
            ...
            uiSystem.navigationObject = navigableObjects.find(
              (o) => o.navigationAutoSelect,
            );
          }
        ...
        if (!uiSystem.navigationTimer.active()) {
          ...
          if (direction) {
            ...
            if (!uiSystem.navigationObject) {
              ...
              newNavigationObject = navigableObjects.find(
                (o) => o.navigationAutoSelect,
              );
              ...
              if (!newNavigationObject) {
                ...
                const newIndex =
                  direction > 0 ? 0 : navigableObjects.length - 1;
                newNavigationObject = navigableObjects[newIndex];
              }
            } else {
              const currentIndex = navigableObjects.indexOf(
                uiSystem.navigationObject,
              );
              const newIndex = mod(
                currentIndex + direction,
                navigableObjects.length,
              );
              newNavigationObject = navigableObjects[newIndex];
            }
    ...
    function uiRender() {
      ...
      if (uiDebug > 0) {
        ...
        function renderDebug(o, visible = true) {
          ...
          for (const c of o.children) renderDebug(c, visible);
        }
  ...
  getNavigableObjects() {
    function getNavigableRecursive(o) {
      if (!o.visible || o.disabled) return; // skip children if parent is invisible or disabled

      if (o.isInteractive() && o.navigationIndex !== undefined) objects.push(o);
    ...
    for (let i = uiSystem.uiObjects.length; i--; ) {
      const o = uiSystem.uiObjects[i];
      if (uiSystem.confirmDialog && o !== uiSystem.confirmDialog) continue;
  ...
  getNavigationDirection() {
    ...
    if (isUsingGamepad) {
      ...
      if (both) return -(stick.y || dpad.y) || stick.x || dpad.x;
    ...
    const up = "ArrowUp",
      ...
      right = "ArrowRight";
    if (both) {
      return keyIsDown(up) || keyIsDown(left)
        ? -1
        : keyIsDown(down) || keyIsDown(right)
          ? 1
          : 0;
    }
  ...
  getNavigationOtherDirection() {
    if (uiSystem.navigationDirection === 2) return 0; // other direction disabled
  ...
  showConfirmDialog(
    ...
    confirmMenu.onRender = () => {
      confirmMenu.pos = uiSystem.screenToNative(mainCanvasSize.scale(0.5));
      const backgroundColor = hsl(0, 0, 0, 0.7);
      uiSystem.drawRect(vec2(), vec2(1e9), backgroundColor);
    };
    confirmMenu.onUpdate = () => {
      if (keyWasPressed(exitKey)) closeMenu();
    };
    ...
    buttonYes.hoverColor = hsl(0, 1, 0.5);
    buttonYes.onClick = () => {
      closeMenu();
...
class UIObject {
  ...
  update() {
    ...
    if (this.disabled) {
      if (this === uiSystem.activeObject) uiSystem.activeObject = undefined;
      if (this === uiSystem.keyInputObject) uiSystem.keyInputObject = undefined;
    }

    if (uiSystem.keyInputObject) return;
      ...
      if (!this.disabled) {
        if (mousePress) {
          if (this.interactive) {
            ...
            if (uiSystem.activateOnPress) this.click(!this.soundPress);
          }
        }
  ...
  render() {
    ...
    const lineColor = isNavigationObject
      ? this.color
      : this.interactive && this.isActiveObject() && !this.disabled
        ? this.color
        : this.lineColor;
    const color = isNavigationObject
      ? this.hoverColor
      : this.disabled
        ? this.disabledColor
        : this.interactive
  ...
  click(playSound) {
    this.onClick();
    if (playSound && this.soundClick) this.soundClick.play();
  }
...
class UITextInput extends UIObject {
  ...
  stopEditing() {
    if (!this.isKeyInputObject()) return;
  ...
  onKeyDown(e) {
    const code = e.code,
      key = e.key;
    if (code === "Backspace") this.text = this.text.slice(0, -1);
    else if (code === "Enter" || code === "Escape") this.stopEditing();
...
class UIScrollbar extends UIObject {
  ...
  update() {
    ...
    if (this.isActiveObject()) {
      ...
      if (!uiSystem.navigationTimer.active())
        this.value = clamp(this.value + direction * 0.01);
    }
  ...
  render() {
    ...
    if (this.fillMode) {
      ...
      const p = (progressWidth - barWidth) * (isHorizontal ? 0.5 : -0.5);
      ...
      const drawSize = isHorizontal
        ? vec2(progressWidth, this.size.y)
        : vec2(this.size.x, progressWidth);
      uiSystem.drawRect(
        pos,
        drawSize,
        color,
      ...
      const p = (barWidth - handleWidth) * (value - 0.5);
      const pos = this.pos.add(isHorizontal ? vec2(p, 0) : vec2(0, p));
      const color = this.disabled ? this.disabledColor : this.handleColor;
...
class Box2dTileLayer extends Box2dStaticObject {
  ...
  constructor(tileLayer) {
    ASSERT(
      tileLayer instanceof TileCollisionLayer,
   ...
   *  @param {number} [friction]
   *  @param {number} [restitution] */
  buildCollision(friction = 0.2, restitution = 0) {
    ...
    for (let x = 0; x < this.size.x; ++x)
      for (let y = 0; y < this.size.y; ++y) {
        ...
        let width = 1,
          height = 1,
          canExpand = true;
        while (isSolidUnprocessed(x + width, y)) ++width;
