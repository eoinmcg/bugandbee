 ...
 *  @param {boolean} [screenSpace]
 *  @memberof Debug */
function debugPoint(pos, color, time, angle, screenSpace = false) { debugRect(pos, undefined, color, time, angle, false, screenSpace); }
 ...
 *  @return {number}
 *  @memberof Math */
function lerp(valueA, valueB, percent) { return valueA + clamp(percent) * (valueB - valueA); }
 ...
 *  @return {number}
 *  @memberof Math */
function percentLerp(value, percentA, percentB, lerpA, lerpB) { return lerp(lerpA, lerpB, percent(value, percentA, percentB)); }
 ...
 *  @return {number}
 *  @memberof Math */
function lerpWrap(valueA, valueB, percent, wrapSize = 1) { return valueA + clamp(percent) * distanceWrap(valueB, valueA, wrapSize); }
 ...
 *  @memberof Math */
function wave(frequency = 1, amplitude = 1, t = time, offset = 0) { return amplitude / 2 * (1 - cos(offset + t * frequency * 2 * PI)); }
 ...
 *  @return {Vector2}
 *  @memberof Random */
function randInCircle(radius = 1, minRadius = 0) { return radius > 0 ? randVec2(radius * rand(minRadius / radius, 1) ** .5) : new Vector2; }
...
class RandomGenerator {
  ...
  *  @param {number} [valueB]
  *  @return {Vector2} */
  vec2(valueA = 1, valueB = 0) { return vec2(this.float(valueA, valueB), this.float(valueA, valueB)); }
...
class Color {
   ...
   * @return {boolean} */
  isValid() { return isNumber(this.r) && isNumber(this.g) && isNumber(this.b) && isNumber(this.a); }
}
 ...
 *  @param {string} [type]
 *  @memberof Utilities */
function saveText(text, filename = 'text', type = 'text/plain') { saveDataURL(URL.createObjectURL(new Blob([text], { 'type': type })), filename); }
...
class EngineObject {
   ...
   *  @param {Vector2} force */
  applyForce(force) { if (this.mass) this.applyAcceleration(force.scale(1 / this.mass)); }
   ...
   *  @param {EngineObject} object
   *  @return {boolean} */
  isOverlappingObject(object) { return this.isOverlapping(object.pos, object.size); }

  /** Check if overlapping a point or aligned bounding box
   *  @param {Vector2} pos          - Center of box
   *  @param {Vector2} [size=vec2()] - Size of box, uses a point if undefined
   *  @return {boolean} */
  isOverlapping(pos, size = vec2()) { return isOverlapping(this.pos, this.size, pos, size); }
...
class TileInfo {
  ...
  *  @return {TileInfo}
  */
  offset(offset) { return new TileInfo(this.pos.add(offset), this.size, this.textureInfo, this.padding, this.bleed); }
  ...
  */
   ...
   * @return {TileInfo}
   */
  tile(index) { return tile(index, this.size, this.textureInfo, this.padding, this.bleed); }
}
 ...
 *  @memberof Input */
function pointerLockRequest() { !isTouchDevice && mainCanvas.requestPointerLock?.(); }

/** Request to unlock the pointer
 *  @memberof Input */
function pointerLockExit() { document.exitPointerLock?.(); }
 ...
 *  @memberof Audio */
function audioIsRunning() { return audioContext.state === 'running'; }
...
class Sound {
   ...
   */
  playMusic(volume = 1, loop = true, paused = false) { return this.play(undefined, volume, 1, 0, loop, paused); }
   ...
   */
  getDuration() { return this.sampleChannels?.[0]?.length / this.sampleRate || 0; }
...
class CanvasLayer extends EngineObject {
  ...
  *  @memberof Draw */
  ...
  updateWebGL() { this.textureInfo.createWebGLTexture(); }

  /** Check if this layer is using WebGL
   *  @return {boolean} */
  hasWebGL() { return glEnable && this.textureInfo.hasWebGL(); }
}
...
class TileLayer extends CanvasLayer {
  ...
  */
   ...
   *  @param {Vector2} layerPos - Local position in array
   *  @param {boolean} [redraw] - Force the tile to redraw if true */
  clearData(layerPos, redraw = false) { this.setData(layerPos, new TileLayerData, redraw) }
   ...
   *  @param {number} [angle] - Angle to rotate by
   */
  drawLayerRect(pos, size, color, angle = 0) { this.drawLayerTile(pos, size, undefined, color, angle); }
...
class TileCollisionLayer extends TileLayer {
  ...
  *  @param {Vector2} layerPos */
  clearCollisionData(layerPos) { this.setCollisionData(layerPos, 0); }
...
class NewgroundsMedal extends Medal {
   ...
   *  @param {string} [src]         - Image location for the medal
   */
  constructor(id, name, description, icon, src) { super(id, name, description, icon, src); }
...
class ZzFXMusic extends Sound {
   ...
   *  @return {SoundInstance} - The sound instance
   */
  playMusic(volume = 1, loop = true) { return super.play(undefined, volume, 1, 0, loop); }
}
 ...
 *  @memberof UISystem */
function uiSetDebug(debugMode) { uiDebug = typeof debugMode === 'boolean' ? (debugMode ? 1 : 0) : debugMode; }
...
class UIVideo extends UIObject {
   ...
   *  @return {boolean} */
  isLoading() { return this.video.readyState < this.video.HAVE_CURRENT_DATA; }
   ...
   *  @return {boolean} */
  isPlaying() { return !this.isPaused() && !this.hasEnded() && !this.isLoading(); }
  ...
  getVideoSize() { return vec2(this.video.videoWidth, this.video.videoHeight); }

  /** Seek to time in seconds
   *  @param {number} time - Time in seconds to seek to */
  setTime(time) { this.video.currentTime = clamp(time, 0, this.getDuration()); }
...
class Box2dObject extends EngineObject {
  ...
  destroyAllFixtures() { this.getFixtureList().forEach(fixture => this.destroyFixture(fixture)); }
   ...
   *  @param {boolean} [isSensor] */
  setSensor(isSensor = true) { this.getFixtureList().forEach(f => f.SetSensor(isSensor)); }
